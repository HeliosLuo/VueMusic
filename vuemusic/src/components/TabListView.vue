<template>
  <div>
    <tabs :currentIndex="currentIndex" @changeCurrentIndex="changeCurrentIndex">
      <tab
        v-for="(element,index) in tabsList"
        :key="index"
        :index="index+1"
      ...
      >
            <div class="panel hotsongs on">
                    <ul class="list">
                        <router-link :to="{ name:'Player',params:{music_id:item.song_id} }" tag="li" class="song url" v-for="(item,index) in element.song_list" :key="index">
                        <div class="poster">
